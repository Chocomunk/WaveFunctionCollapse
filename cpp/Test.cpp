...
int main(int argc, char** argv) {
	assert(argc == 4);
	char* tiles_dir = argv[1];
	int set = atoi(argv[2]); // should be 0 if single image, 1 if it is a set of tiles
	int render = atoi(argv[3]); // should be 0 if we don't want to render, 1 if we do
